...
async def send_code(
   ...
   try:
        ...
        async with aiofiles.open(f"code-{globalId}.tex", "w", encoding="utf-8", newline="") as f:
            await f.write(payload)
        if imageFile:
            async with aiofiles.open(f"image-{globalId}.{image_format}", "wb") as f:
               await f.write(await imageFile.read())
    
        await asyncio.to_thread(subprocess.run,["pdflatex", f"-jobname=pdf-{globalId}", "-interaction=nonstopmode", f"code-{globalId}.tex"])
        
        file_path = f"pdf-{globalId}.pdf"
        if not os.path.exists(file_path):
            raise HTTPException(status_code=500, detail="Failed to generate resume. Please try again.")
